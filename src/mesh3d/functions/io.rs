...
pub fn load_obj_manifold(filename: &str) -> Result<ManifoldTriangularMesh3D> {
    ...
    ManifoldTriangularMesh3D::new(vertices, faces)
}

/// Loads off file as manifold mesh
pub fn load_off_manifold(filename: &str) -> Result<ManifoldTriangularMesh3D> {
    let mut vertices = Vec::new();
    let mut faces = Vec::new();
